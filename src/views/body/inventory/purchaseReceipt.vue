<template>
  <div class="body">
    <!--搜索-->
    <div class="search">
      <el-input v-model="listPage.PlsScheduleNo" style="width: 280px;" clearable placeholder="请扫入单号"
                size="small" @keyup.enter.native="handleFilter"></el-input>
      <el-button size="small" type="primary" @click="handleFilter" :disabled="listPage.PlsScheduleNo == null || listPage.PlsScheduleNo === ''">搜索</el-button>
    </div>
    ...
    <van-pull-refresh v-model="isLoading" @refresh="onRefresh" success-text="刷新成功" class="content" v-if="contentShow">
      <div class="content-header">
        ...
        <el-select v-model="listPage.SupplierId" size="small" filterable placeholder="请选择供应商">
          <el-option
              v-for="item in supplierSlectList"
              :key="item.Id"
              :label="item.Name"
              :value="item.Id">
          </el-option>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        api.getReceiptnoticeItem(this.$qs.stringify(param)).then(res => {
          this.show = false
          let data = res.data;
      ...
      allSave() {
        Dialog.confirm({
          ...
          message: '该操作将提交当前页的所有数据，是否继续？'
            ...
            .then(() => {
              let StkPurStockInDetails = []
              for (let item of this.list) {
                if (item.num === 0) {
                  continue;
                }
                StkPurStockInDetails.push({
                  FromBillNo: item.PlsScheduleNo,
                  FromRowId: item.PlsScheduleRowId,
              ...
              const param = {
                ...
                StkPurStockInDetails
              }
              api.addstkpurstockin(this.$qs.stringify((param))).then(res => {
                if (res.data.Success) {

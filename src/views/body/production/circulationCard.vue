      ...
      <div class="content-item">
        <transition-group name="van-slide-up">
          <ul v-for="(item,index) of list" :key="index">
            ...
            <li>完工数量：{{item.CompletionQualifiedQuantity}}</li>
            <li>备注：{{item.Remark}}</li>
            <li class="opt_right">
              <el-button size="small" type="success" @click="handleWatchMore(item.BillNo)">查看详情</el-button>
            </li>
          </ul>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        api.getprocessflowitem(this.$qs.stringify(param)).then(res => {
          this.show = false
          let data = res.data;

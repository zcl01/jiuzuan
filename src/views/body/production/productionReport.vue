      ...
      <div class="content-item">
        <transition name="van-slide-up">
          <ul>
            ...
            <li>剩余数量：{{list.RemainingQty}}</li>
            <li class="dis_flex_a">报工数量：
              <van-stepper v-model="list.Quantity" input-width="100px" button-size="32px" :min="0" :max="list.RemainingQty"/>
            </li>
          </ul>
        </transition>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        api.getprocessflowreportworkitem(this.$qs.stringify(param)).then(res => {
          this.show = false
          let data = res.data;
          ...
          message: '该操作将提交当前页的所有数据，是否继续？'
            ...
            .then(() => {
              const param = {
                ...
                MaxQuantity: this.list.RemainingQty,
                Quantity: this.list.Quantity,
                PersonId: this.listPage.PersonId,
                CreatorId: this.listPage.PersonId,
              }
              api.addstkreportwork(this.$qs.stringify((param))).then(res => {
                if (res.data.Success) {

<template xmlns="">
  <div class="body">
    ...
    <van-pull-refresh
        ...
        @refresh="onRefresh"
        success-text="刷新成功"
        class="content" v-if="contentShow">
      <div class="content-header">
        <el-button size="small" type="success" @click="handleSubmit">提交</el-button>
      </div>
      <div class="content-item">
        <ul>
          <li>条码号：{{list.BarCode}}</li>
          <li>采购单：{{list.PlsScheduleNo}}</li>
          <li>供应商：{{list.SupplierName}}</li>
          <li>存货：{{list.MaterialName}}</li>
          <li>规格：{{list.RuleType}}</li>
          <li>样品数量：{{list.SampleQuantity}}{{list.UnitName}}</li>
          <li>检验数量：{{list.CheckQuantity}}{{list.UnitName}}</li>
          <li class="dis_flex_a">
            合格数量：
            <van-stepper
                v-model="list.StandardQuantity"
                input-width="100px"
                button-size="40px"
                :min="0"/>
          </li>
          <li class="dis_flex_a">
            不合格数量：
            <van-stepper
                v-model="list.UnstandardQuantity"
                input-width="100px"
                button-size="40px"
                :min="0"/>
          </li>
          ...
          <li>
            <ol v-for="(item,index) of list.IncomingInspectionDetails" :key="index">
              ...
              <li class="dis_flex_a">
                检验判断：
                <el-select
                    v-model="item.DtestingConclusion"
                    size="small"
                    filterable
                    clearable
                    placeholder="请选择">
  ...
  export default {
    ...
    created() {
      if(this.$route.query.code){
        this.listPage.BillNo = this.$route.query.code;
        this.getList()
      }
    },
    methods: {
      ...
      getList() {
        ...
        api.getincominginspectionitem(this.$qs.stringify(param)).then(res => {
          this.show = false;
          let data = res.data;
      ...
      handleSubmit() {
        this.list.FormBillNo = this.list.BarCode;
        this.list.InspectionQuantity = this.list.CheckQuantity;
        this.list.StkIncomingInspectionDetails = this.list.IncomingInspectionDetails;
        delete this.list.BarCode;
        delete this.list.CheckQuantity;
        delete this.list.IncomingInspectionDetails;
        ...
        Dialog.confirm({
          ...
          message: '该操作将保存当前页的所有数据',
            ...
            .then(() => {
              ...
              api.addstkincominginspection(this.$qs.stringify((param))).then(res => {
                if (res.data.Success) {
